...
void man()
{
	printf("STMicroelectronics UART Flash Loader command line v2.1.0 \n\n");
	printf(" Usage : \n\n");
    ...
    printf("     --ec  echo        : value ON or OFF ..., default is OFF \n");
	...
	printf("  -p        (Activate or diasactivate protections) \n");
			...
			else if (strcmp(argv[arg_index],"-c")==0)
			{
			   while(arg_index < argc)
			   {
				 ...
				 else if(Is_SubOption(argv[arg_index])) // Get connection settings
				 {
				     ...
				     else if (strcmp(argv[arg_index-1],"--ec")==0)  becho  = StatusToBool(argv[arg_index]); // Echo back mode, default is OFF \n");
					   ...
					   if(Ini.IsKeyExist((LPCTSTR)"Product",(LPCTSTR)m_Version))
					   {
						   ...
						   if(!E_W_ROUTINEs.IsEmpty())
						   {
							   ...
							   else
							   {
									...
									{
										 printf("Unable to load data from this file " + ToFind + " ...");
										 return 9;
									}
							   }
						   }
